...
int main(int argc, char **argv) {
    SDL_Window *window = SDL_CreateWindow("CRAZY ARCADE", SDL_WINDOWPOS_CENTERED, 30, SCREEN_WIDTH,
                                          SCREEN_HEIGHT, 0);
    ...
    while (!Exit) {
        ...
        switch (mainMenu(renderer)) {
            case SINGLEPLAYER:
                CHARACTER = chooseSingleCharacter(renderer);
                if (Exit || CHARACTER == 0)
                    break;
                Mix_HaltMusic();
            ...
            case MULTIPLAYER:
                while (mode == 0 || (CHARACTER_.x == 0 && CHARACTER_.y == 0)) {
    ...
    while (!Char) {
        ...
        while (SDL_PollEvent(&event)) {
            ...
            if (event.type == SDL_QUIT) {
                Exit = true;
                return 0;
            } else if (charBoz.checkArea(event.button.x, event.button.y)) {
                hovered = &boz;
                hoveredRect = &bozRect;
                if (gameRender::waitMouseDown(event)) {
                    if (soundOn)
        ...
        if (hovered != 0) {
            if (curTime - preTime > 10) {
                preTime = curTime;
                hovered->setBlendTexture(255);
                if (hovered == &boz || hovered == &plunk || hovered == &ike || hovered == &evie) {
                    arrowRect.x = hoveredRect->x + 40;
...
void mainSingleGame(SDL_Renderer *renderer, int CHARACTER) {
    ...
    do {
        ...
        } while (blockMap[lockBlock.y][lockBlock.x] != BRICK &&
                 blockMap[lockBlock.y][lockBlock.x] != GIFT_BOX);
        while (!quit && level == 1) {
            while (SDL_PollEvent(&event))
                if (event.type == SDL_QUIT) {
                    Exit = true;
                    return;
                            ...
                            bPause.checkArea(event.button.x, event.button.y)) ||
...
void mainMultiGame_classic(SDL_Renderer *renderer, SDL_Point CHARACTER) {
    ...
    do {
        ...
        switch (rand() % 4) {
            case 0:
                player1.setPosition(TOP_LEFT);
                player2.setPosition(BOT_RIGHT);
                break;
            case 1:
                player1.setPosition(TOP_RIGHT);
                player2.setPosition(BOT_LEFT);
                break;
            case 2:
                player1.setPosition(BOT_LEFT);
                player2.setPosition(TOP_RIGHT);
                break;
            case 3:
                player1.setPosition(BOT_RIGHT);
...
int renderGameOver(SDL_Renderer *renderer, int winner) {
    ...
    switch (winner) {
        case 0:
            gameOver.renderString(BLACK, "GAME OVER");
            if (soundOn)
                Mix_PlayChannel(-1, levelFail_sound, 0);
            break;
        case 1:
            gameOver.renderString(GREEN, "PLAYER 1 WIN");
            if (soundOn)
                Mix_PlayChannel(-1, win_sound, 0);
            break;
        case 2:
            gameOver.renderString(RED, "PLAYER 2 WIN");
            if (soundOn)
                Mix_PlayChannel(-1, win_sound, 0);
            break;
        case 3:
            gameOver.renderString(BLUE, "YOU WIN");
            if (soundOn)
                Mix_PlayChannel(-1, win_sound, 0);
            break;
    }
...
void pauseGame(SDL_Renderer *renderer, bool &quit, int currentMap) {
    ...
    while (pausing) {
        ...
        while (SDL_PollEvent(&event)) {
            if (event.type == SDL_QUIT) {
                ...
                if (gameRender::waitMouseDown(event)) {
                    if (soundOn) {
                        ...
                        if (currentMap == S_TOWN || currentMap == M_TOWN || currentMap == L_MAP1 ||
                            currentMap == L_MAP2)
            ...
            } else if ((bContinue.checkArea(event.button.x, event.button.y) &&
                        gameRender::waitMouseDown(event)) ||
                       (event.type == SDL_KEYDOWN && event.key.keysym.sym == SDLK_ESCAPE))
                pausing = false;
        }
        SDL_RenderPresent(renderer);
...
void tutorial(SDL_Renderer *renderer) {
    ...
    while (!quit) {
        ...
        while (SDL_PollEvent(&event)) {
            ...
            } else if (bNext.checkArea(event.button.x, event.button.y) && page == 1) {
                hovered = &Next;
                if (gameRender::waitMouseDown(event)) {
                    if (soundOn)
                        Mix_PlayChannel(1, button_press, 0);
            ...
            } else if (bPrevious.checkArea(event.button.x, event.button.y) && page == 2) {
                hovered = &Previous;
                if (gameRender::waitMouseDown(event)) {
                    if (soundOn)
                        Mix_PlayChannel(1, button_press, 0);
